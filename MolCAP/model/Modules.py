...
class Encoder(nn.Module):
    ...
    :return: a list of the atom embeddings of the last layer shape (batch_size, num_atoms, d_model)
    """
    __constants__ = ['norm']
    ...
    def forward(self, x, attn_mask=None, need_weights=False):
        ...
        for mod in self.layers:
            output, attn = mod(output, attn_mask, need_weights=need_weights)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, attn_mask=None, need_weights=False):
        ...
        if self.norm_first:
            x_new, att = self._sa_block(self.norm1(x), attn_mask, need_weights=need_weights)
            x = x + x_new
            x = x + self._ff_block(self.norm2(x))
        else:
            x_new, att = self._sa_block(x, attn_mask, need_weights=need_weights)
            x = self.norm1(x + x_new)
            x = self.norm2(x + self._ff_block(x))

        return x, att

    def _sa_block(self, x, attn_mask, need_weights=False):
...
class Decoder(nn.Module):
    ...
    def forward(self, tgt, memory, attn_mask=None):
        output = tgt
        for mod in self.layers:
            output = mod(tgt, memory, attn_mask)
...
class MultiHeadAtomAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None, need_weights=False):
        ...
        attn_output = F.linear(attn_output, self.out_proj.weight, self.out_proj.bias)  # 将attn_output与self.out_proj的权重矩阵进行点乘，并将偏置向量加上，得到一个新的输出矩阵
